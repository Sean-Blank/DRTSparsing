...
class StructConstraints:
	...
	def update(self, ix, state):
		...
		elif self.kb_start <= ix <= self.kb_end:
			state.stack.append(self.kb_start)
			state.stack_ex.append([0 for i in range(2)])
			state.k += 1
		elif self.pb_start <= ix <= self.pb_end:
...
class RelationConstraintsState:
	def __init__(self):
		pass
	def reset(self):
		self.rel_g = 0
...
class VariableConstraintsState:
	def __init__(self):
		pass
...
def get_constraints(args, action_vocab):
	...
	for line in open(args.action_dict_path):
		line = line.strip()
		if line == "###":
			starts.append(action_vocab.toidx(lines[0]))
			ends.append(action_vocab.toidx(lines[-1]))
			lines = []
		else:
			if line[0] == "#":
				continue
	...
	cons_step1 = StructConstraints(action_vocab, args)
	cons_step2 = RelationConstraints(action_vocab, args, starts, ends)
	cons_step3 = VariableConstraints(action_vocab, args)
	return [cons_step1, cons_step2, cons_step3]
